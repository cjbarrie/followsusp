...
suspended_followers_all <- readRDS("data/output/suspended_followers_all.rds")

# take just 3 for testing
suspended_followers_all <- suspended_followers_all[1:3] #TODO change to all in production
...
for (i in seq_along(suspended_followers_all)) {
  ...
  data_path_for_follower_tweets = paste0("data/testdata/", suspended_account) #TODO change from testdata folder when in production
  ...
  #sample followers
  set.seed(123L)
  followers_sample <- sample(followers, 10, replace = F) #TODO change to full sample or sample of max n. in production
  
  #get tweets of followers
  for(j in seq_along(followers_sample)) {
    
    #TODO build in error handling here
